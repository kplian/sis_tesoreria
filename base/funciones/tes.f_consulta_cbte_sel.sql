CREATE
    OR REPLACE FUNCTION tes.f_consulta_cbte_sel(
        ...
        begin
            ...
            IF
                ...
                IF v_id_funcionarios IS NOT NULL THEN
                    v_filtro =
                            ' (lower(incbte.estado_reg)!=''borrador'') and (lower(incbte.estado_reg)!=''validado'' ) and ';
                ELSE
                    v_filtro = ' (ew.id_funcionario=' || v_parametros.id_funcionario_usu::varchar ||
                               ') and (lower(incbte.estado_reg)!=''anulado'') and (lower(incbte.estado_reg)!=''borrador'') and (lower(incbte.estado_reg)!=''validado'' ) and ';
                END IF;
            ELSE
                v_filtro =
                        ' (lower(incbte.estado_reg)!=''borrador'') and (lower(incbte.estado_reg)!=''validado'' ) and ';
            ...
            v_consulta
                := 'select  incbte.manual,
                          ...
                          left join tes.tplan_pago pan on pan.id_int_comprobante = incbte.id_int_comprobante
