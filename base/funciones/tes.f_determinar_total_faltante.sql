...
BEGIN
    ...
    END IF;
    ----#MSA-31 Datos de plan de pago
    SELECT
        t.estado,
        t.tipo,
        t.descuento_anticipo
    INTO
        v_record
    FROM tes.tplan_pago t
    WHERE t.id_plan_pago = p_id_plan_pago ;
    ...
    ELSEIF  p_filtro = 'ant_parcial_descontado' THEN
        ...
        -- #ETR-2703 se comenta por que ahora se toma el devengado como base y ya no los pagos
/*                      select
                         sum(pp.descuento_anticipo)
                      into
                         v_monto_aux
...
*/
        ...
        IF v_record.estado = 'devengado' and v_record.tipo = 'devengado_pagado' THEN
            select
                sum(pp.descuento_anticipo)
            into
                v_monto_ant_descontado
            from tes.tplan_pago pp
            where  pp.estado_reg='activo'
              and pp.tipo in('pagado')          --un supeusto es que los descuentos de anticipo solo se hacen en el comprobante de pago
              and pp.id_plan_pago_fk = p_id_plan_pago;
