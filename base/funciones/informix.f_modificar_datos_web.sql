...
BEGIN
   ...
   DROP FOREIGN TABLE IF EXISTS informix.boletos_fpago_modificacion;
    ...
    if (v_id_boleto is null and v_estado = '1') then
    	v_error = 'No se puede actualizar la informacion de ventas web porque no existe el boleto ' || p_billete;
    ...
    if (v_detalle.medio_pago != 'COMPLETAR-CC')  then
        if (v_estado = '1') then
            ...
            if (v_detalle.importe != v_forma_pago.importe) then
                v_error = 'El importe de la forma de pago no iguala con el importe de la venta web para el boleto ' || p_billete;
            	v_id_alarma = (select param.f_inserta_alarma_dblink (1,'Error al actualizar ingresos desde la venta web',v_error,'jaime.rivera@boa.bo'));

            end if;

            if (v_detalle.moneda != v_forma_pago.codigo_internacional) then
                v_error = 'La moneda de la forma de pago no iguala con la moneda de la venta web para el boleto ' || p_billete;
            	v_id_alarma = (select param.f_inserta_alarma_dblink (1,'Error al actualizar ingresos desde la venta web',v_error,'jaime.rivera@boa.bo'));
                raise exception '%',v_error;
            end if;
